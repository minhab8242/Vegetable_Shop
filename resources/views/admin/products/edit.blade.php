        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    @if($product->image_url)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $product->image_url) }}" alt="{{ $product->name }}" class="h-20 w-20 object-cover rounded-lg">
                            <p class="text-xs text-gray-500 mt-1">Ảnh hiện tại</p>
                        </div>
                    @endif
                    <input type="file" id="image" name="image" accept="image/*"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('image') border-red-500 @enderror">
                    <p class="text-xs text-gray-500 mt-1">Định dạng: JPEG, PNG, JPG, GIF. Tối đa 2MB. Để trống nếu không thay đổi</p>
                    @error('image')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="price" name="price" value="{{ old('price', number_format($product->price, 0, ',', '.')) }}"
                ...
                <div>
                    ...
                    <input type="text" id="cost_price" name="cost_price" value="{{ old('cost_price', $product->cost_price ? number_format($product->cost_price, 0, ',', '.') : '') }}"
